...
### Загрузить пример (Куб-ая И.В.)

```bash
make seed-kub
```

Создаст семью Куба-их, пациента, 3 визита (УЗИ ОБП + 2 офтальмолога), наблюдения (ВГД, VA), 5 активных проблем и 9 личных задач.
...
| `make seed-kub` | засеять данные Куба-ой И.В. (идемпотентно) |
